            ...
            function filterPartituras() {
                ...
                if (document.querySelector('.instrumento-content')) {
                    document.querySelectorAll('.border-b').forEach(section => {
                        ...
                        section.querySelectorAll('[data-obra-titulo]').forEach(obraElement => {
                            const obraTitulo = obraElement.getAttribute('data-obra-titulo').toLowerCase();
                            const matchesSearch = obraTitulo.includes(searchTerm);

                            // Mostrar/ocultar obra individual
                            obraElement.style.display = matchesSearch || !searchTerm ? 'flex' : 'none';
                        ...
                        section.style.display = hasMatchingObras || !searchTerm ? 'block' : 'none';

                        // Actualizar el contador de partituras disponibles
                        const countElement = section.querySelector('.text-sm.text-gray-500');
                        if (countElement) {
                            if (searchTerm) {
                ...
                if (document.querySelector('.autor-content')) {
                    document.querySelectorAll('.border-b').forEach(autorSection => {
                        let hasMatchingObras = false;
                        let matchingCount = 0;

                        // Buscar en obras del autor
                        autorSection.querySelectorAll('[data-obra-titulo]').forEach(obraElement => {
                            const obraTitulo = obraElement.getAttribute('data-obra-titulo').toLowerCase();
                            const matchesSearch = obraTitulo.includes(searchTerm);

                            // Mostrar/ocultar obra individual
                            obraElement.style.display = matchesSearch || !searchTerm ? 'block' : 'none';
                        ...
                        autorSection.style.display = hasMatchingObras || !searchTerm ? 'block' : 'none';

                        // Actualizar el contador de obras disponibles
                        const countElement = autorSection.querySelector('.text-sm.text-gray-500');
                        if (countElement) {
                            if (searchTerm) {
