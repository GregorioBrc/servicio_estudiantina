<x-app-layout title="Editar Instrumento">
    <div class="min-h-screen bg-white text-gray-900 p-6 flex items-center justify-center">
        <div class="max-w-md w-full bg-white p-8 rounded-lg shadow-lg border border-gray-200">
            <h1 class="text-3xl font-bold mb-6 text-center text-gray-900">Editar Instrumento: {{ $instrumento->nombre }}</h1>

            <form action="{{ route('admin.instrumentos.update', $instrumento) }}" method="POST" class="space-y-4">
                @csrf
                @method('PUT')

                <div>
                    <label for="nombre" class="block text-sm font-medium text-gray-700">Nombre del Instrumento:</label>
                    <input type="text" name="nombre" id="nombre" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 text-gray-900" required value="{{ old('nombre', $instrumento->nombre) }}">
                    @error('nombre')
                ...
                <div>
                    ...
                    <input type="text" name="tipo" id="tipo" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 text-gray-900" required value="{{ old('tipo', $instrumento->tipo) }}">
                    @error('tipo')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
